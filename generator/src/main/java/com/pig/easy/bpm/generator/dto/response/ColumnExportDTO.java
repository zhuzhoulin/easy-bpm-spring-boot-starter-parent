package com.pig.easy.bpm.generator.dto.response;

import com.baomidou.mybatisplus.annotation.TableName;
import com.baomidou.mybatisplus.annotation.IdType;
import com.baomidou.mybatisplus.extension.activerecord.Model;
import com.baomidou.mybatisplus.annotation.Version;
import com.baomidou.mybatisplus.annotation.TableId;
import java.time.LocalDateTime;
import com.baomidou.mybatisplus.annotation.FieldFill;
import com.baomidou.mybatisplus.annotation.TableField;
import java.io.Serializable;
import lombok.Data;
import lombok.EqualsAndHashCode;
import lombok.experimental.Accessors;
import com.alibaba.excel.annotation.*;
import java.io.Serializable;
import java.util.Date;
import java.math.*;
/**
 * <p>
    * 字段表
    * </p>
 *
 * @author pig
 * @since 2021-03-11
 */
@Data
@EqualsAndHashCode(callSuper = false)
@Accessors(chain = true)
public class ColumnExportDTO implements Serializable {

    @ExcelIgnore
    private static final long serialVersionUID = 1L;

    /**
     * 列序号
     */
    @ExcelProperty( value = "列序号")
    private Long  columnId;

    /**
     * 数据源ID
     */
    @ExcelProperty( value = "数据源ID")
    private Long  dbId;

    /**
     * 表名
     */
    @ExcelProperty( value = "表名")
    private String  tableName;

    /**
     * 字段名
     */
    @ExcelProperty( value = "字段名")
    private String  columnName;

    /**
     * 编码
     */
    @ExcelProperty( value = "编码")
    private String  collation;

    /**
     * 额外参数
     */
    @ExcelProperty( value = "额外参数")
    private String  extra;

    /**
     * 权限
     */
    @ExcelProperty( value = "权限")
    private String  privileges;

    /**
     * 默认值
     */
    @ExcelProperty( value = "默认值")
    private String  defaultValue;

    /**
     * 字段描述
     */
    @ExcelProperty( value = "字段描述")
    private String  columnComment;

    /**
     * 标签名称
     */
    @ExcelProperty( value = "标签名称")
    private String  formLabel;

    /**
     * Java字段类型
     */
    @ExcelProperty( value = "Java字段类型")
    private String  propertyType;

    /**
     * 字段属性名
     */
    @ExcelProperty( value = "字段属性名")
    private String  propertyName;

    /**
     * 字段类型
     */
    @ExcelProperty( value = "字段类型")
    private String  columnType;

    /**
     * 主键 1 是 0 否
     */
    @ExcelProperty( value = "主键 1 是 0 否")
    private Integer  keyFlag;

    /**
     * 自增列 1 是 0 否
     */
    @ExcelProperty( value = "自增列 1 是 0 否")
    private Integer  keyIdentityFlag;

    /**
     * 必填 1 是 0 否
     */
    @ExcelProperty( value = "必填 1 是 0 否")
    private Integer  notNull;

    /**
     * 查询条件 1 是 0 否
     */
    @ExcelProperty( value = "查询条件 1 是 0 否")
    private Integer  queryFlag;

    /**
     * 查询方式
     */
    @ExcelProperty( value = "查询方式")
    private String  queryMethod;

    /**
     * 插入是否显示 1 是 0 否
     */
    @ExcelProperty( value = "插入是否显示 1 是 0 否")
    private Integer  insertFlag;

    /**
     * 更新是否显示 1 是 0 否
     */
    @ExcelProperty( value = "更新是否显示 1 是 0 否")
    private Integer  updateFlag;

    /**
     * 查询是否显示 1 是 0 否
     */
    @ExcelProperty( value = "查询是否显示 1 是 0 否")
    private Integer  listFlag;

    /**
     * 下载是否显示 1 是 0 否
     */
    @ExcelProperty( value = "下载是否显示 1 是 0 否")
    private Integer  downloadFlag;

    /**
     * 校验类型
     */
    @ExcelProperty( value = "校验类型")
    private String  validateType;

    /**
     * 校验 最大长度
     */
    @ExcelProperty( value = "校验 最大长度")
    private String  validateMaxLength;

    /**
     * 校验 最小长度
     */
    @ExcelProperty( value = "校验 最小长度")
    private String  validateMinLength;

    /**
     * 校验 最大值
     */
    @ExcelProperty( value = "校验 最大值")
    private String  validateMaxValue;

    /**
     * 校验 最小值
     */
    @ExcelProperty( value = "校验 最小值")
    private String  validateMinValue;

    /**
     * 表单类型
     */
    @ExcelProperty( value = "表单类型")
    private String  formType;

    /**
     * 关联字典编码
     */
    @ExcelProperty( value = "关联字典编码")
    private String  dictCode;

    /**
     * 关联字典名称
     */
    @ExcelProperty( value = "关联字典名称")
    private String  dictName;

    /**
     * 租户编号
     */
    @ExcelProperty( value = "租户编号")
    private String  tenantId;

    /**
     * 备注
     */
    @ExcelProperty( value = "备注")
    private String  remarks;

    /**
     * 状态 1 有效 0 失效
     */
    @ExcelProperty( value = "状态 1 有效 0 失效")
    private Integer  validState;

    /**
     * 操作人工号
     */
    @ExcelProperty( value = "操作人工号")
    private Integer  operatorId;

    /**
     * 操作人姓名
     */
    @ExcelProperty( value = "操作人姓名")
    private String  operatorName;

    /**
     * 创建时间
     */
    @ExcelProperty( value = "创建时间")
    private LocalDateTime  createTime;

    /**
     * 更新时间
     */
    @ExcelProperty( value = "更新时间")
    private LocalDateTime  updateTime;
}
